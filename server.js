...
const app = express();
// Northflank termina HTTPS na borda e encaminha os cabeçalhos X-Forwarded-* para o contêiner.
...
function postgresPoolConfig(connectionString) {
    ...
    // No Northflank, TLS do addon PostgreSQL é opcional. Não force SSL apenas
    // porque o host não é localhost: respeite o indicador do addon ou sslmode da URI.
    const tlsFlag = String(process.env.DATABASE_TLS_ENABLED ?? process.env.TLS_ENABLED ?? '').trim().toLowerCase();
    if (['true','1','yes','on'].includes(tlsFlag)) {
        const rejectUnauthorized = String(process.env.DATABASE_SSL_REJECT_UNAUTHORIZED || 'false').toLowerCase() === 'true';
        config.ssl = { rejectUnauthorized };
        return config;
    }
    if (['false','0','no','off'].includes(tlsFlag)) {
        config.ssl = false;
        return config;
    }
    try {
        ...
        else if (['require','verify-ca','verify-full'].includes(mode || '')) {
            config.ssl = { rejectUnauthorized: mode !== 'require' };
        }
    } catch (_) { }
    return config;
}
...
async function initStorage() {
    if (!disablePostgres && process.env.DATABASE_URL && Pool) {
        pgPool = new Pool(postgresPoolConfig(process.env.DATABASE_URL));
        await pgPool.query(`
...
function scheduleJsonSave() {
    ...
    // volume persistente do provedor, este arquivo sobrevive aos deploys. Mesmo com
...
io.on('connection', (socket) => {
    ...
    socket.on('join-room', async (data) => {
        try {
            ...
            const nextDeviceIndex = Math.max(0, ...alreadyOnline.map(p => Number(p.deviceIndex) || 0)) + 1;
            ...
            // Apenas UM aparelho por jogador pode publicar câmera/microfone/tela.
            // O primeiro aparelho conectado vira a fonte inicial. Um segundo aparelho NUNCA
            // toma a mídia automaticamente: ele entra somente como receptor até o usuário
            // escolher explicitamente trocar a fonte na aba Sala.
            const selectedSource = room.mediaSources?.[ownerKey];
            const selectedStillOnline = Boolean(selectedSource && room.players[selectedSource]);
            if (!selectedStillOnline) room.mediaSources[ownerKey] = socket.id;
    ...
    function selectMediaSourceForOwner(ownerKey, targetSocketId, automatic=false) {
        ...
        // Ao trocar a fonte, todos os outros endpoints do mesmo jogador ficam
        // imediatamente em modo somente-recepção. Música/SFX não passam por esta
        // regra porque são sincronizados por eventos separados da chamada WebRTC.
        for (const endpoint of roomOwnerEndpoints(room, ownerKey)) {
            if (endpoint.id === targetSocketId) continue;
            endpoint.cameraOn = false;
            endpoint.micOn = false;
            endpoint.screenSharing = false;
            io.to(code).emit('media-state', {
                id:endpoint.id, cameraOn:false, micOn:false, screenSharing:false
            });
        }
    ...
    socket.on('media-state', (data) => {
        ...
        room.mediaSources ||= {};
        if (!room.mediaSources[player.ownerKey]) room.mediaSources[player.ownerKey] = socket.id;
        const isSelectedSource = room.mediaSources[player.ownerKey] === socket.id;

        // Defesa no servidor: um aparelho secundário não pode anunciar câmera,
        // microfone ou compartilhamento como ativos. O cliente também bloqueia o
        // envio de tracks, mas esta validação mantém o estado da sala coerente.
        if (!isSelectedSource) {
            const attemptedPublish = data.cameraOn === true || data.micOn === true || data.screenSharing === true;
            player.cameraOn = false;
            player.micOn = false;
            player.screenSharing = false;
            if (attemptedPublish) {
                socket.emit('media-source-enforced', {
                    ownerKey:player.ownerKey,
                    socketId:room.mediaSources[player.ownerKey]
                });
            }
        } else {
...
(async () => {
    ...
    server.listen(PORT, HOST, () => {
        console.log(`🚀 Servidor RPG Mesa em ${HOST}:${PORT}`);
