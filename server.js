...
// ─────────────────────────────────────────────────────────────
// CLOUDFLARE REALTIMEKIT — chamada multiponto via SFU
// O Socket.IO continua sincronizando a mesa. Áudio/vídeo deixam de depender
// da malha P2P quando as credenciais RealtimeKit estão configuradas.
// Se RealtimeKit falhar ou não estiver configurado, o cliente mantém o WebRTC
// P2P antigo como fallback para não impedir a entrada na campanha.
// ─────────────────────────────────────────────────────────────
const RTK_API_ROOT = 'https://api.cloudflare.com/client/v4';
const rtkMeetingPromises = new Map();
let rtkPresetPromise = null;

function realtimeKitConfigured() {
    return Boolean(
...
function realtimeKitApiPath(suffix='') {
    const account = encodeURIComponent(String(process.env.CLOUDFLARE_ACCOUNT_ID || '').trim());
    const app = encodeURIComponent(String(process.env.CLOUDFLARE_REALTIMEKIT_APP_ID || '').trim());
    return `/accounts/${account}/realtime/kit/${app}${suffix}`;
}
async function realtimeKitApi(suffix, options={}) {
    if (!realtimeKitConfigured()) throw new Error('RealtimeKit não configurado no servidor.');
    const res = await fetch(RTK_API_ROOT + realtimeKitApiPath(suffix), {
        method: options.method || 'GET',
        headers: {
            'Authorization': `Bearer ${String(process.env.CLOUDFLARE_REALTIMEKIT_API_TOKEN || '').trim()}`,
    ...
    if (!res.ok || payload?.success === false) {
        const message = payload?.errors?.[0]?.message || payload?.messages?.[0]?.message || `HTTP ${res.status}`;
        const err = new Error(`Cloudflare RealtimeKit: ${message}`);
...
async function resolveRealtimeKitPreset(role='jogador') {
    const roleSpecific = role === 'narrador'
        ? String(process.env.CLOUDFLARE_REALTIMEKIT_HOST_PRESET || '').trim()
        : String(process.env.CLOUDFLARE_REALTIMEKIT_PLAYER_PRESET || '').trim();
    if (roleSpecific) return roleSpecific;
    const explicit = String(process.env.CLOUDFLARE_REALTIMEKIT_PRESET || '').trim();
    if (explicit) return explicit;
    if (!rtkPresetPromise) {
        rtkPresetPromise = (async () => {
            const presets = await realtimeKitApi('/presets?per_page=100');
            const list = Array.isArray(presets) ? presets : [];
            // O app criado no Dashboard recebe presets padrão. Priorizamos um
            // preset de group call para garantir que todos possam falar e usar vídeo.
            const preferred = list.find(p => String(p?.name || '').toLowerCase() === 'group-call-host')
                || list.find(p => /group[-_ ]?call/i.test(String(p?.name || '')))
                || list[0];
            if (!preferred?.name) throw new Error('Nenhum preset RealtimeKit foi encontrado.');
            return preferred.name;
        })().catch(err => { rtkPresetPromise = null; throw err; });
    }
    return rtkPresetPromise;
}
async function ensureRealtimeKitMeeting(room, code, forceNew=false) {
    if (!realtimeKitConfigured()) return null;
    if (!forceNew && room.realtimeKitMeetingId) return room.realtimeKitMeetingId;
    if (rtkMeetingPromises.has(code)) return rtkMeetingPromises.get(code);
    const promise = (async () => {
        const meeting = await realtimeKitApi('/meetings', {
            method:'POST',
            body:{ title:`Mesa RPG · ${String(room.campaignName || room.roomName || code).slice(0,120)}` }
        });
        if (!meeting?.id) throw new Error('RealtimeKit não retornou o ID da reunião.');
        room.realtimeKitMeetingId = meeting.id;
        await persistRoom(code);
        return meeting.id;
    })().finally(() => rtkMeetingPromises.delete(code));
    rtkMeetingPromises.set(code, promise);
    return promise;
}
async function createRealtimeKitJoin(room, code, socket, player) {
    if (!realtimeKitConfigured()) return { enabled:false, mode:'p2p-fallback', reason:'not-configured' };
    const add = async (meetingId) => {
        const presetName = await resolveRealtimeKitPreset(player.role);
        const participant = await realtimeKitApi(`/meetings/${encodeURIComponent(meetingId)}/participants`, {
            method:'POST',
            body:{
                name: String(player.name || 'Jogador').slice(0,80),
                preset_name: presetName,
                // Um participante por endpoint mantém o mapeamento exato entre
                // RealtimeKit e socket.id, inclusive quando PC + celular estão online.
                custom_participant_id: socket.id
            }
        });
        if (!participant?.token || !participant?.id) throw new Error('RealtimeKit não retornou token do participante.');
        socket.data.realtimeKitParticipantId = participant.id;
        socket.data.realtimeKitMeetingId = meetingId;
        return {
            enabled:true,
    ...
    try {
        const meetingId = await ensureRealtimeKitMeeting(room, code, false);
        return await add(meetingId);
    } catch (firstError) {
        // Se a sala persistiu um meeting antigo que foi removido no Dashboard,
        // cria outro meeting uma única vez. Outros erros continuam no fallback.
        if (room.realtimeKitMeetingId) {
            try {
                room.realtimeKitMeetingId = '';
                await persistRoom(code);
                const meetingId = await ensureRealtimeKitMeeting(room, code, true);
                return await add(meetingId);
            } catch (secondError) {
                console.error('⚠️ RealtimeKit indisponível; usando WebRTC P2P:', secondError.message);
                return { enabled:false, mode:'p2p-fallback', reason:'realtimekit-error' };
            }
        }
        console.error('⚠️ RealtimeKit indisponível; usando WebRTC P2P:', firstError.message);
        return { enabled:false, mode:'p2p-fallback', reason:'realtimekit-error' };
    }
}
async function deleteRealtimeKitParticipant(meetingId, participantId) {
    if (!meetingId || !participantId || !realtimeKitConfigured()) return;
    try {
        await realtimeKitApi(`/meetings/${encodeURIComponent(meetingId)}/participants/${encodeURIComponent(participantId)}`, { method:'DELETE' });
    } catch (err) {
        console.warn('RealtimeKit: não foi possível remover participante desconectado:', err.message);
    }
}
...
function normalizeRoom(room, code) {
    ...
    room.realtimeKitMeetingId = String(room.realtimeKitMeetingId || '').slice(0,120);
...
app.get('/health', (req, res) => {
    res.status(200).json({
        ...
        storage: storageMode,
        videoProvider: realtimeKitConfigured() ? 'cloudflare-realtimekit' : 'webrtc-p2p-fallback',
        realtimeKitConfigured: realtimeKitConfigured(),
...
io.on('connection', (socket) => {
    ...
    socket.on('join-room', async (data) => {
        try {
            ...
            // Token de mídia é emitido apenas para o próprio socket. A chave da API
            // Cloudflare nunca é exposta ao navegador.
            const realtimeKit = await createRealtimeKitJoin(room, code, socket, room.players[socket.id]);
    ...
    socket.on('disconnect', () => {
        ...
        deleteRealtimeKitParticipant(socket.data.realtimeKitMeetingId, socket.data.realtimeKitParticipantId).catch(()=>{});
